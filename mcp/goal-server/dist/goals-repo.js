...
import { BUDGET_PROFILES, PRACTICAL_UNBOUNDED_CONTINUATIONS, PRACTICAL_UNBOUNDED_WALL_CLOCK_SECONDS, isBudgetProfileInput, resolveBudgetProfile, } from "./budget-profiles.js";
...
export class GoalsRepo {
    ...
    recordVerdict(session_id, goal_id, verdict, reason = null, evidence = null) {
        const txn = this.db.transaction(() => {
    ...
    hasRecentEvaluatorVerdict(session_id, goal_id, windowMs = EVALUATOR_VERDICT_WINDOW_MS) {
      ...
      WHERE session_id = ? AND goal_id = ?
        ...
        AND created_at_ms >= ?
    `).get(session_id, goal_id, Date.now() - windowMs);
        return row.n > 0;
    }
    markComplete(session_id, goal_id, completedBy = "self_update") {
        const txn = this.db.transaction(() => {
            ...
            if (completedBy === "evaluator" && !this.hasRecentEvaluatorVerdict(session_id, g.goal_id)) {
                throw new Error("completed_by:'evaluator' requires a recent recorded evaluator verdict. Dispatch the claude-goal:goal-evaluator subagent; it must call the record_verdict tool with verdict 'complete' before you call update_goal. If the evaluator is unavailable, use completed_by:'self_update' instead (not allowed from budget_limited or accounting_error states).");
            }
            const evaluatorCanCloseAccountingPause = g.status === "paused" && g.paused_reason === "accounting_error" && completedBy === "evaluator";
            const evaluatorCanCloseBudgetLimited = g.status === "budget_limited" && completedBy === "evaluator";
      ...
      `).run(elapsedSec, now, session_id, g.goal_id);
            ...
            if (g.status !== "active")
                payload.from_status = g.status;
            if (g.paused_reason)
                payload.paused_reason = g.paused_reason;
    ...
    updateObjective(session_id, goal_id, objective) {
        const txn = this.db.transaction(() => {
            const g = this.getBySession(session_id);
            if (!g)
                throw new Error("no goal exists");
      ...
      `).run(objective, now, session_id, g.goal_id);
            this.recordEvent(session_id, g.goal_id, "goal_objective_updated", g.status, g.status, { previous_objective: g.objective });
        });
        txn();
    }
