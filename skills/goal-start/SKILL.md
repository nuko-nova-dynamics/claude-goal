...
Before marking the goal complete, build and verify a prompt-to-artifact checklist showing every requirement is met. Then dispatch the plugin subagent `claude-goal:goal-evaluator` with the Agent tool (Task is an older alias if Agent is unavailable). Pass the session id, objective, checklist, transcript path if known, and concrete evidence. The evaluator verifies with tools and records its verdict via the `record_verdict` MCP tool — never call `record_verdict` yourself. If it returns `{"verdict":"complete"}`, call `update_goal` with `status: "complete"` and `completed_by: "evaluator"`. If it returns `{"verdict":"impossible"}`, call `update_goal` with `status: "blocked"` and the evaluator's reason.
...
If the user explicitly changes what the goal should accomplish mid-run, call `update_objective` with the new objective instead of abandoning and recreating the goal; budgets and token accounting carry over.
