---
title: claude-goal
description: Goal-bounded autonomous turns for Claude Code. Say "set up a goal" or use /goal-start, pick a budget profile when you need one, walk away.
template: splash
hero:
  tagline: A production-grade autonomous loop for Claude Code — built around a Stop-hook continuation engine and a tool-equipped completion evaluator. Say "set up a goal" or use /goal-start, pick a budget profile when you need one, walk away.
  actions:
    - text: Quickstart
...
<Tabs>
  ...
  <TabItem label="Tarball">
    ...
    tar -xzf claude-goal-v0.2.5.tar.gz \
...
```
set up a goal to list every .ts file under src/ and print a line count for each
```
...
`standard` gives the run 2M tokens, 75 continuations, and 4 hours. Omit `--budget` on `/goal-start` for an unbounded goal. Explicit prose goal starts use `auto` unless you ask for another profile, raw token budget, or unbounded mode. See [Budgets and caps](/claude-goal/concepts/budgets/) for the full profile table.
