...
For a budget-bounded run:

```
/goal-start "refactor the auth module to use async/await" --budget 50000
```

When `(tokens_used + subagent_tokens) >= 50000`, the hook transitions the goal to `budget_limited` and emits a one-shot budget-limit prompt. Use `/goal-extend --add-continuations N` to raise the cap and resume.

## Deep dive

<CardGrid>
  <LinkCard title="How it works" href="/concepts/how-it-works/" description="Stop-hook continuation, token accounting, completion paths, F5 final-turn retry, SQLite state store." />
  <LinkCard title="Budgets and caps" href="/concepts/budgets/" description="Token budgets, turn caps, wall-clock caps. Worker vs subagent attribution." />
  <LinkCard title="Completion evaluator" href="/concepts/evaluator/" description="Why a tool-equipped subagent beats same-model self-audit." />
  <LinkCard title="State and persistence" href="/concepts/state/" description="The SQLite schema, migration runner, audit event log." />
</CardGrid>
