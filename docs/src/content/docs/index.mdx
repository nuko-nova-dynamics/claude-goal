...
<Tabs>
  ...
  <TabItem label="Tarball">
    ...
    tar -xzf claude-goal-v0.2.7.tar.gz \
...
`standard` gives the run 10M tokens, 200 continuations, and 8 hours. Omit `--budget` on `/goal-start` for practical-unlimited token, turn, and wall-clock budget. Explicit prose goal starts use `auto` unless you ask for another profile, raw token budget, or unbounded mode. See [Budgets and caps](/claude-goal/concepts/budgets/) for the full profile table.
