---
title: claude-goal
description: Goal-bounded autonomous turns for Claude Code. Set an objective, set a budget, walk away.
template: splash
hero:
  tagline: A production-grade autonomous loop for Claude Code — built around a Stop-hook continuation engine and a tool-equipped completion evaluator. Set an objective, set a budget, walk away.
  actions:
    - text: Quickstart
...
When `(tokens_used + subagent_tokens) >= 3000000`, the hook transitions the goal to `budget_limited` and emits a one-shot budget-limit prompt. See [Budgets and caps](/claude-goal/concepts/budgets/) for how to size a budget for the shape of your goal.
